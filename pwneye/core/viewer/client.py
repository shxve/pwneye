...
from PyQt6.QtCore import QEvent, QPoint, QTimer, QUrl, pyqtSignal, Qt
...
class MultiChannelViewer(QWidget):
    ...
    def __init__(self, attempts: list[RtspAttempt]) -> None:
        super().__init__()
    ...
    def start(self) -> None:
        ...
        if self.single_stream_mode and self.tiles:
            QTimer.singleShot(0, self._enter_single_stream_focus)
    ...
    def eventFilter(self, obj, event) -> bool:  # type: ignore[override]
        if event.type() != QEvent.Type.KeyPress or not self.isVisible() or not self.isActiveWindow():
